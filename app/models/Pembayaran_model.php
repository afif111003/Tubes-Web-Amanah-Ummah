...
$jumlah_tanggungan = isset($data['jumlah_tanggungan']) ? $data['jumlah_tanggungan'] : '';
$pembayaran_beras = isset($data['pembayaran_beras']) ? $data['pembayaran_beras'] : '';
$pembayaran_uang = isset($data['pembayaran_uang']) ? $data['pembayaran_uang'] : '';

// Validasi untuk memastikan kolom tidak kosong
$errors = [];

if (empty($jumlah_tanggungan)) {
    $errors[] = "Kolom jumlah tanggungan tidak boleh kosong!";
}

if (empty($pembayaran_beras) && empty($pembayaran_uang)) {
    $errors[] = "Salah satu kolom pembayaran (beras atau uang) harus diisi!";
}

// Menampilkan pesan peringatan jika ada kesalahan
if (!empty($errors)) {
...
$jumlah_tanggungan_numeric = intval($jumlah_tanggungan);
$pembayaran_beras_numeric = floatval($pembayaran_beras);
$pembayaran_uang_numeric = floatval($pembayaran_uang);

// Logika bisnis setelah validasi berhasil
if ($pembayaran_uang == '0' || $pembayaran_uang == '') {
    $total_pembayaran = $jumlah_tanggungan_numeric * $pembayaran_beras_numeric;
} elseif ($pembayaran_beras == '0' || $pembayaran_beras == '') {
    $pembayaran_uang_per_orang = $pembayaran_uang_numeric / $jumlah_tanggungan_numeric;
    $total_pembayaran = $pembayaran_uang_numeric;
}
		...
		if($jumlahnya=='0'){
			...
			VALUES (:id_pembayar, :nama_pembayar, :no_hp_pembayar, :alamat_pembayar,  :id_masjid)";
