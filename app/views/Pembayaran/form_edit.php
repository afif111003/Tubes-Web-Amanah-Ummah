      ...
      <input type="hidden" id="last-name" name="id_pembayaran" required="required" class="form-control" value="<?= $dt['id_pembayaran']?>">
        <h3><center>Data Pembayar</center></h3>
          ...
          <div class="form-group-inner">
              <div class="row">
                  <div class="col-lg-3">
                      <label class="login2 pull-right pull-right-pro">Masjid</label>
                  </div>
                  <div class="col-lg-9">
                      <div class="form-select-list">
                          <select name="id_masjid" class="form-control">
                              <option>Pilih Masjid</option>
                              <?php foreach( $data['Masjid'] as $dt2 ) : ?>
                              <?php 
                                  $selected = '';
                                  if($dt2['id_masjid'] == $dt['id_masjid']){
                                  $selected = 'selected="selected"';
                                  }
                              ?>
                              <option value="<?= $dt2['id_masjid'];?>" <?= $selected;?>>
                              <?= $dt2['nama_masjid'];?></option>
                          <?php endforeach; ?>
                          </select>
                      </div>
                  </div>
